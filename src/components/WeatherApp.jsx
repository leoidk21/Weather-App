...
const WEATHER_ICONS = {
    ...
    // Thunderstorms (Slight, Moderate, Heavy with Hail)
    95: storm, 
    96: storm, 
    99: storm
};
...
function WeatherApp() {
    ...
    // toggle handler function for Units Menu and Days Menu dropdown
    const toggleMenu = () => setIsMenuOpen(!isMenuOpen);
    const toggleDaysMenu = () => setIsDaysMenuOpen(!isDaysMenuOpen);

    // Weather App states
    const [searchQuery, setSearchQuery] = useState(''); 
    const [locationName, setLocationName] = useState('Berlin, Germany'); // set location state, default to Berlin Germany
    ...
    const fetchWeather = async (city) => {
        ...
        try {
            // --- GEO CODING ---
            /*  This is Open-Meteo's built-in geocoding endpoint to translate names to coordinates 
                Convert City Name into Latitude/Longitude Coordinates */
            ...
            // If the API returns an empty list, the city doesn't exist
    ...
    // Load initial data on mount or when the unit tracking switch changes
    useEffect(() => {
        fetchWeather(locationName.split(',')[0]);
    }, [unitSystem]); // If unitSystem changes between metric/imperial, it auto-refetches the numbers
    ...
    return (
        <div className="weather-page-container">
                    ...
                    <div className="unit-dropdown-wrapper">
                        {isMenuOpen && (
                            <div className="unit-dropdown-menu">
                                <button className='unit-dropdown-item'>Switch to Imperial</button>
    
                                <p>Temperature</p>
    
                                <button className='unit-dropdown-item'>Celsius (°C)</button>
                                <button className='unit-dropdown-item'>Fahrenheit (°F)</button>
                                
                                <div className="divider"></div>
    
                                <p>Wind Speed</p>
    
                                <button className='unit-dropdown-item'>km/h</button>
                                <button className='unit-dropdown-item'>mph</button>
    
                                <div className="divider"></div>
    
                                <p>Precipitation</p>
    
                                <button className='unit-dropdown-item'>Millimeters (mm)</button>
                                <button className='unit-dropdown-item'>Inches (in)</button>
                            </div>
                        )}
                    </div>
                </section>
            ...
            <section className="main-section">
                ...
                {loading && <p className="status-message">Loading local atmospheric metrics...</p>}
